...
export function AuthProvider({children}:{children:React.ReactNode}) {
 ...
 async function signUp(email:string,password:string,data?:UniversalSignupData){setError(null);if(configError)return{error:configError,userId:null,hasSession:false,needsEmailVerification:false};try{const supabase=getSupabaseBrowserClient();const {data:result,error:signUpError}=await supabase.auth.signUp({email,password,options:{data:data?{full_name:data.fullName,phone:data.phone,country:data.country,state:data.state,gender:data.gender,date_of_birth:data.dateOfBirth}:undefined}});if(signUpError){setError(signUpError.message);return{error:signUpError.message,userId:null,hasSession:false,needsEmailVerification:false};}return{error:null,userId:result.user?.id??null,hasSession:!!result.session,needsEmailVerification:!result.session};}catch(error){const message=error instanceof Error?error.message:String(error);setError(message);return{error:message,userId:null,hasSession:false,needsEmailVerification:false};}}
